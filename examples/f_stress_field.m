...
maxtaurr = max(max(taurr)); %max(abs(taurr(:))); %1; %max(max(abs(taurr)));
...
clevels = 1000;
addpath('./')
[logtaurr1,logtaurr,logr1,logr2] = f_gradedstress(r_coord',R',Req,R0,Ca,Ca1,l1,l2,v_nc,v_a);
maxlogtaurr = 1; %max(max(taurr)); %max(abs(taurr(:))); %1; %max(max(abs(taurr)));
...
ya.LineWidth = 1.5;
box on;
plot(t, log(R),'LineWidth',3,'Color','k');
contourf(xcon,ycon,logntaurr,clevels,'edgecolor','none')
hold on;
plot(t,logr1','b--','LineWidth',2,'DisplayName','l1')
plot(t,logr2','r--','LineWidth',2,'DisplayName','l2')
