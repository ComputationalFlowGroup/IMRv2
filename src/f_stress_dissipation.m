...
function [taudivu] = f_stress_dissipation(stress,spectral,Req,R,R1,R2,Rdot,Ca,Ca1,Br, ...
    Re8,alphax,yT2,yT3,iyT3,iyT4,iyT6,X,ZZT,ivisco1,ivisco2,fnu,DRe,a)
...
elseif stress == 8
    taudivu = 12*(Br/(Re8+DRe*fnu))*(Rdot/R)^2*iyT6 + ...
        2*Br/Ca*iyT3.*(Rdot/R).*(yT2.*ix2 - iyT4.*x4) + ...
        2*Br/Ca*dGdr0nd*iyT3.*(Rdot/R).*(yT3.*ix3 - iyT3.*x3);

    %gm quadratic Kelvin-Voigt neo-Hookean solid with linearly varying G
elseif stress == 9
    taudivu = 12*(Br/(Re8+DRe*fnu))*(Rdot/R)^2*iyT6 + ...
        2*Br/Ca*iyT3.*(Rdot/R).*(yT2.*ix2 - iyT4.*x4) .* ...
        (1+alphax*(x4.*iyT4 + 2*yT2.*ix2 - 3)) + ...
