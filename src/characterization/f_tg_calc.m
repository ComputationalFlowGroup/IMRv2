...
%        Ee2 = @(x) arrayfun(@(xi) integral(@(y) ee2integ(y,xi),x1(xi),x2(xi),'RelTol',reltol,'AbsTol',abstol),x); %integrating wrt lambda
        Ee2 = @(x) arrayfun(@(xi) ...
             (abs(x1(xi) - x2(xi)) < 1e-10) * 0 + ... % return 0 if bounds are effectively equal
             (abs(x1(xi) - x2(xi)) >= 1e-10) * integral(@(y) ee2integ(y, xi), x1(xi), x2(xi), 'RelTol', reltol, 'AbsTol', abstol), ...
              x);
 %       Eem2 = @(x) arrayfun(@(xi) integral(@(y) ee2integ(y,xi),xm1,xm2,'RelTol',reltol,'AbsTol',abstol),x); %integrating wrt lambda
       Eem2 = @(x) arrayfun(@(xi) ...
             (abs(x1(xi) - x2(xi)) < 1e-10) * 0 + ... % return 0 if bounds are effectively equal
             (abs(x1(xi) - x2(xi)) >= 1e-10) * integral(@(y) ee2integ(y, xi), xm1, xm2, 'RelTol', reltol, 'AbsTol', abstol), ...
              x);
