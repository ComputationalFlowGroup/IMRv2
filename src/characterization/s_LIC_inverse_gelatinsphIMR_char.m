...
addpath Synthetic_data\ ../forward_solver/ ../common/ ../../../Experimental_data/UM_data/
% load("NSLIC_processed.mat")
load("dataset2_IMR.mat")
...
G = 2.61e3; sigma = 0.04; rho = 1048;
%G = 2e3;
perturbed = 0; % turns on characterization for surface perturbations, 1 is on

%% prep experimental data for the optimization
...
for i = gel
    ...
    t = t - t(1);
    fps = length(t)/t(end);
    kindata{k} = struct('time', t.*tc, 'R', R, 'Req', Req, 'epnm', 0,...
        'epnmd0', 0, 'n', 2, 'm', 2, 'G', G, 'alpha', 0, ...
        'mu', 0, 'ST', sigma, 'fps', fps,'rho', rho);
end
...
for s = 1
    ...
    x_data = struct('time', texp, 'n', n, 'Req', Req, 'chi', chinm, 'ST', ST, ...
        'rho', rho, 'Gqs', Gqs, 'epnmd0', 0, 'epnm0', 0, 'R0', Rmax, ...
        'idx_col', idx_col, 'perturbed', 0, 'pertmod',0,'aR', 1, 'aEP', 1);
    figure
    ...
    % end
     ...
     objfun = @(params, x_data) f_run_fd_IMR_sph(10.^params, x_data);
