...
#define ETHRESH 1.0e-6

#define MAX_ITERATIONS 10000

#define MACHEP 1.4e-15
...
double a, b, c, x;
{
  ...
  hypdon:
    if (err > ETHRESH) {
		...
		mexWarnMsgIdAndTxt("mexhyp2f1:hyp2f1","Error threshold not reached.");
    }
    return (y);
  ...
  hypdiv:
	...
	mexWarnMsgIdAndTxt("mexhyp2f1:hyp2f1","Overflow range error.");
    return NPY_INFINITY;
}
...
double *loss;
{
	...
	if (fabs(d - id) > EPS) {
		...
		if (t > MAX_ITERATIONS) {	/* should never happen */
			...
			mexWarnMsgIdAndTxt("mexhyp2f1:hyp2f1","Too many iterations.");
		    *loss = 1.0;
		    return NPY_NAN;
		}
	    }
		       ...
		       double *loss)
{
    ...
    *loss = 0;
    ...
    /*assert(da != 0);*/
    if (da == 0)
        mexErrMsgIdAndTxt("mexhyp2f1:hyp2f1ra","da is zero.");

    if (fabs(da) > MAX_ITERATIONS) {
        /* Too expensive to compute this value, so give up */
        /* mtherr("hyp2f1", TLOSS);*/
		mexWarnMsgIdAndTxt ("mexhyp2f1:hyp2f1ra","Too expensive to compute this value, so give up.");
        *loss = 1.0;
        return NPY_NAN;
    }
